...
def flash_attention(
    ...
    q_offsets=None, # [batch_size]
    q_idx=None, # [batch_size, q_seq_len]
    k_idx=None, # [batch_size, kv_seq_len]
    *,
    causal: bool = False,
    sm_scale: float = 1.0,
  ...
  if num_heads_k != num_heads_v or num_heads % num_heads_k != 0:
    raise ValueError(
        f"Num heads mismatch: got {num_heads}, {num_heads_k} and"
  ...
  if q_offsets is not None:
    if not causal:
      raise ValueError(
          f"Q offsets is not None: expected q_offsets to be None"
          f" when {causal=}, but q_offsets is not None"
      )
    if block_sizes is not None:
      if block_sizes.block_b != 1 and not jnp.all(jnp.equal(q_offsets, q_offsets[0])):
        raise NotImplementedError(
            f"{block_sizes.block_b=} != 1 not supported when q_offsets is not None"
            f" and q_offsets != q_offsets[0]"
        )
    if q_offsets.shape != (batch_size, ):
      raise ValueError(
          f"Q offsets shape mismatch: expected ({batch_size}, ),"
          f" got {q_offsets.shape}"
      )
  if q_idx is not None or k_idx is not None:
    if q_idx is None or k_idx is None:
      raise ValueError(
          f"Received None: expected either both or none of"
          f" k_idx to be none, got q_idx as"
          f" {q_idx if q_idx is None else 'not None'}"
          f" and k_idx as"
          f" {k_idx if k_idx is None else 'not None'}"
      )
    if q_idx.shape != (batch_size, q_seq_len):
      raise ValueError(
        f"Q idx shape mismatch: expected ({batch_size},"
        f" {q_seq_len}), got {q_idx.shape}"
      )
    if k_idx.shape != (batch_size, kv_seq_len):
      raise ValueError(
        f"K idx shape mismatch: expected ({batch_size},"
        f" {kv_seq_len}), got {k_idx.shape}"
      )
  ...
  return _flash_attention(
      q, k, v, ab, segment_ids, q_offsets, q_idx, k_idx, False, causal, sm_scale, block_sizes, debug
  )


@functools.partial(jax.custom_vjp, nondiff_argnums=range(8, 13))
...
def _flash_attention_fwd(
  ...
  o, l, m = _flash_attention(
      q, k, v, ab, segment_ids, q_offsets, q_idx, k_idx, True, causal, sm_scale, block_sizes, debug
  )
  return o, (q, k, v, ab, segment_ids, q_offsets, q_idx, k_idx, o, l, m)
...
def _flash_attention_bwd(
  ...
  if save_residuals:
    raise NotImplementedError("Higher-order AD not supported")
  (q, k, v, ab, segment_ids, q_offsets, q_idx, k_idx, o, l, m) = residuals
...
def below_or_on_diag(r, r_blk_size, c, c_blk_size, q_offset=0):
  # A block is considered below or on diagonal as long as the bottom left
  # corner of the block is below or on diagonal.
  return ((r + 1) * r_blk_size - 1 + q_offset) > (c * c_blk_size)


def _flash_attention_kernel(q_offsets_ref, q_tile_ref, *args, **kwargs):
  ...
  for batch_idx in range(block_b):
    kernel((batch_idx, 0), q_offsets_ref, q_tile_ref, *args, **kwargs)
...
def _flash_attention_kernel_single_batch(
  ...
  global_batch_idx = pl.program_id(0)
  batch_size = q_tile_ref.shape[0]
  if causal:
    q_offset = 0
    if q_offsets_ref is not None:
      q_offset = q_offsets_ref[global_batch_idx * batch_size + batch_idx[0]]

    should_run = below_or_on_diag(q_seq_idx, block_q, kv_seq_idx, block_k_major, q_offset)
  else:
    should_run = True
  ...
  def run():
    ...
    def _body(start_k):
      ...
      if q_idx_tile_ref is not None:
        ...
        q_idx = pltpu.repeat(
            q_idx_tile_ref[batch_idx[0]], repeats, axis=1
        )  # [block_q, block_k].
        k_idx = k_idx_tile_ref[
            batch_idx[0], :1, pl.dslice(start_k, block_k)
        ]  # [1, block_k].
        attention_mask = jnp.logical_and(q_idx, k_idx).astype(jnp.bool_)
        mask = (
            attention_mask if mask is None else jnp.logical_and(mask, attention_mask)
        )

      if causal:
        mask_shape = (block_q, block_k)
        row_ids = jax.lax.broadcasted_iota(jnp.int32, mask_shape, 0)
        row_ids += q_seq_idx * block_q + q_offset
      ...
      row_has_any = jnp.ones((block_q, )) if mask is None else jnp.any(mask, axis=-1)
      ...
      p = jnp.exp(
        s - jnp.where(
          row_has_any[:, None],
          pltpu.repeat(m_next, block_k_repeats, 1),
          0.0
        )
      )
      ...
      l_corr = alpha * l_prev
      l_next = jnp.sum(p, axis=1)[:, None] + l_corr  # [block_q, 1] (broadcasted)
...
def _flash_attention_kernel_single_batch_single_step(
  ...
  if q_idx_tile_ref is not None:
    ...
    q_idx = q_idx_tile_ref[
        batch_idx[0]
    ]  # [block_q, NUM_LANES].
    q_idx = pltpu.repeat(
        q_idx, repeats, axis=1
    )  # [block_q, block_k].
    k_idx = k_idx_tile_ref[batch_idx[0], :1]  # [1, block_k].
    attention_mask = jnp.logical_and(q_idx, k_idx).astype(jnp.bool_)
    mask = attention_mask if mask is None else jnp.logical_and(mask, attention_mask)
  ...
  p = jnp.exp(s - jnp.where(row_has_any[:, None], m, 0.0))
  l = jnp.sum(p, axis=1)[:, None]
  
  p = jnp.where(l > 0.0, p / l, 0.0)
...
def _flash_attention_impl(
  ...
  def q_index_map(batch_index, head_index, q_seq_index, _, *prefetch_refs):
    del prefetch_refs
    return (batch_index, head_index, q_seq_index, 0)

  def kv_index_map(batch_index, head_index, q_seq_index, kv_seq_index, q_offsets_ref):
    if causal:
      # If the kv block is skipped, prefetch the next valid kv block, i.e. the
      # 0th one to be used for the next block_q rows.
      q_offset = 0
      if q_offsets_ref is not None:
        q_offset = q_offsets_ref[batch_index]
      next_kv_index = lax.select(
          below_or_on_diag(q_seq_index, block_q, kv_seq_index, block_k_major, q_offset),
          kv_seq_index,
          0,
      )
    else:
      next_kv_index = kv_seq_index
    return (batch_index, head_index // g, next_kv_index, 0)

  def ab_index_map(batch_index, head_index, q_seq_index, kv_seq_index, q_offsets_ref):
    if causal:
      q_offset = 0
      if q_offsets_ref is not None:
        q_offset = q_offsets_ref[batch_index]
      should_run = below_or_on_diag(
          q_seq_index, block_q, kv_seq_index, block_k_major, q_offset
      )
  ...
  def o_index_map(batch_index, head_index, q_seq_index, _, *prefetch_refs):
    del prefetch_refs
    return (batch_index, head_index, q_seq_index, 0)

  def lm_index_map(batch_index, head_index, q_seq_index, _, *prefetch_refs):
    del prefetch_refs
    return (batch_index, head_index, q_seq_index, 0)
  ...
  if segment_ids is not None:
    ...
    def q_segment_ids_index_map(batch_index, head_index, q_seq_index, _, *prefetch_refs):
      del head_index, prefetch_refs
      return (batch_index, q_seq_index, 0)

    def kv_segment_ids_index_map(
        batch_index, head_index, q_seq_index, kv_seq_index, q_offsets_ref
    ):
      del head_index
      if causal:
        q_offset = 0
  ...
  q_idx_spec = k_idx_spec = None
  if q_idx is not None:
    def q_idx_index_map(batch_index, head_index, q_seq_index, _, *prefetch_refs):
      del head_index, prefetch_refs
      return (batch_index, q_seq_index, 0)
    ...
    q_idx_spec = pl.BlockSpec(
        (block_b, block_q, NUM_LANES), q_idx_index_map
    )
    k_idx_spec = pl.BlockSpec(
        (block_b, NUM_SUBLANES, block_k_major), k_idx_index_map
    )

    q_idx = jax.lax.broadcast_in_dim(
        q_idx,
        (batch_size, q_seq_len, NUM_LANES),
        (
            0,
    ...
    k_idx = jax.lax.broadcast_in_dim(
        k_idx,
        (batch_size, NUM_SUBLANES, kv_seq_len),
        (
            0,
  ...
  )(q_offsets, q, k, v, ab, q_segment_ids, kv_segment_ids, q_idx, k_idx)
  if save_residuals:
...
def _flash_attention_dkv_kernel(
  ...
  q_offset = 0
  if q_offsets_ref is not None:
    q_offset = q_offsets_ref[batch_idx]
  ...
  def q_body(j, _):
    ...
    def k_body(i, _):
      ...
      if causal:
        mask_shape = (block_q, block_k)
        row_ids = jax.lax.broadcasted_iota(jnp.int32, mask_shape, 0)
        row_ids += q_seq_index * block_q_major + start_q + q_offset
        col_ids = jax.lax.broadcasted_iota(jnp.int32, mask_shape, 1)
  ...
  if causal:
    should_run = below_or_on_diag(
        q_seq_index, block_q_major, kv_seq_index, block_k_major, q_offset
    )
  else:
    should_run = True
...
def _flash_attention_bwd_dkv(
  ...
  def qo_index_map(batch_index, head_index, kv_seq_index, q_seq_index, q_offsets_ref):
    if causal:
  ...
  def kv_index_map(batch_index, head_index, kv_seq_index, _, *prefetch_refs):
    del prefetch_refs
    return (batch_index, head_index // g, kv_seq_index, 0)
  ...
  def lm_index_map(batch_index, head_index, _, q_seq_index, *prefetch_refs):
    del prefetch_refs
    return (batch_index, head_index, q_seq_index, 0)
  ...
  def ab_index_map(batch_index, head_index, kv_seq_index, q_seq_index, *prefetch_refs):
    del prefetch_refs
  ...
  if segment_ids is not None:
    ...
    def q_segment_ids_index_map(
        batch_index, head_index, kv_seq_index, q_seq_index, q_offsets_ref
    ):
      del head_index
      if causal:
        q_offset = 0
    ...
    def kv_segment_ids_index_map(batch_index, head_index, kv_seq_index, _, *prefetch_refs):
      del head_index, prefetch_refs
  ...
  if q_idx is not None:
    ...
    def k_idx_index_map(batch_index, head_index, kv_seq_index, _, *prefetch_refs):
      del head_index, prefetch_refs
      return (batch_index, 0, kv_seq_index)

    q_idx_spec = pl.BlockSpec(
        (1, block_q_major, NUM_LANES), q_idx_index_map
    )
    k_idx_spec = pl.BlockSpec(
        (1, NUM_SUBLANES, block_k_major), k_idx_index_map
    )
  ...
  out_shapes = [
      jax.ShapeDtypeStruct((batch_size, num_kv_heads, kv_seq_len, head_dim),
                           k.dtype),
      jax.ShapeDtypeStruct((batch_size, num_kv_heads, kv_seq_len, head_dim),
                           v.dtype),
  ]
  def dkv_index_map(batch_index, head_index, kv_seq_index, _, *prefetch_refs):
    del prefetch_refs
    return (batch_index, head_index // g, kv_seq_index, 0)
  ...
  with jax.named_scope(name_scope):
    ...
    )(q_offsets, q, k, v, ab, q_segment_ids, kv_segment_ids, q_idx, k_idx, l, m, do, di)
...
def _flash_attention_dq_kernel(
  ...
  batch_index = pl.program_id(axis=0)
  ...
  def body(i, _):
    ...
    if q_idx_tile_ref is not None:
      ...
      q_idx = pltpu.repeat(
          q_idx_tile_ref[0], repeats, axis=1
      )  # [block_q, block_k].
      k_idx = k_idx_tile_ref[:, 0, k_slice]  # [1, block_k].
      attention_mask = jnp.logical_and(q_idx, k_idx).astype(jnp.bool_)
      mask = (
          attention_mask if mask is None else jnp.logical_and(mask, attention_mask)
      )
    ...
    if causal:
      ...
      row_ids = jax.lax.broadcasted_iota(jnp.int32, mask_shape, 0)
      row_ids += q_seq_index * block_q_major + q_offset
      col_ids = jax.lax.broadcasted_iota(jnp.int32, mask_shape, 1)
    ...
    row_has_any = jnp.ones((block_q_major, )) if mask is None else jnp.any(mask, axis=-1)
    p = jnp.exp(
        capped_logits - jnp.where(
            row_has_any[:, None],
            pltpu.repeat(m, block_k // MIN_BLOCK_SIZE, axis=1),
...
def _flash_attention_bwd_dq(
  ...
  def qo_index_map(batch_index, head_index, q_seq_index, _, *prefetch_refs):
    del prefetch_refs
    return (batch_index, head_index, q_seq_index, 0)
  ...
  def ab_index_map(batch_index, head_index, q_seq_index, kv_seq_index, *prefetch_refs):
    del prefetch_refs
    return (batch_index, head_index, q_seq_index, kv_seq_index)
...
def mha_reference_no_custom_vjp(
  ...
  _, n_heads, q_seq_len, _ = q.shape
  _, n_kv_heads, kv_seq_len, _ = k.shape
  q = jnp.reshape(q, (q.shape[0], n_kv_heads, n_heads//n_kv_heads, *q.shape[2:]))
  ...
  out = jnp.reshape(out, (q.shape[0], n_heads, q_seq_len, q.shape[4]))
  if save_residuals:
    return out, l, m
  return out
...
def mha_reference_bwd(
  ...
  q_gqa = jnp.reshape(q, (batch_size, n_kv_heads, n_groups, q_seq_len, head_dim))
  do_gqa = jnp.reshape(do, (batch_size, n_kv_heads, n_groups, q_seq_len, head_dim))
  o_gqa = jnp.reshape(o, (batch_size, n_kv_heads, n_groups, q_seq_len, head_dim))

  logits = jnp.einsum(
      "bhgqd,bhkd->bhgqk",  # q=q_seq_len, k=kv_seq_len, d=head_dim
      q_gqa.astype(jnp.float32),
  ...
  if segment_ids is not None:
    mask = segment_ids.q[:, :, None] == segment_ids.kv[:, None, :]
    mask = mask[:, None, None, :, :]  # Add head and group dimensions
  ...
  dv = jnp.einsum(
      "bhgqk,bhgqd->bhkd", p, do_gqa.astype(jnp.float32)
  ).astype(v.dtype)

  dp = jnp.einsum(
      "bhgqd,bhkd->bhgqk", do_gqa.astype(jnp.float32), v.astype(jnp.float32)
  )

  di = jnp.sum(
      o_gqa.astype(jnp.float32) * do_gqa.astype(jnp.float32),
      axis=-1,
      keepdims=True
  )
  ds = (dp - di) * p

  dk = jnp.einsum("bhgqd,bhgqk->bhkd", q_gqa.astype(jnp.float32), ds)
  dq_gqa = jnp.einsum("bhgqk,bhkd->bhgqd", ds, k.astype(jnp.float32))
  ...
  dq = jnp.reshape(dq_gqa, q.shape).astype(q.dtype)
